...
/*
  Com os quatro tokens vazios em lib/site-config.ts, o layout nao emite o
  script do aviso: nao ha medidor para gatilhar, entao nao ha aviso. Sem este
  desvio o teste falharia 26 vezes anunciando um defeito que nao existe --
  falha de teste tem que significar defeito, nao configuracao ausente.
*/
if (!fs.readFileSync(path.join(RAIZ, 'index.html'), 'utf8').includes('consentimento-valen.js')) {
  console.log('Nenhum medidor configurado: o aviso nao e emitido e nao ha o que testar.')
  console.log('Preencha GA_MEASUREMENT_ID, AHREFS_ANALYTICS_KEY ou CLOUDFLARE_ANALYTICS_TOKEN')
  console.log('em site/lib/site-config.ts, reconstrua, e rode de novo.')
  servidor.close()
  process.exit(0)
}
